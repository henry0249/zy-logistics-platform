    ...
    <div style="padding:0 3%">
      <my-table border size="mini" height="100vh - 50px" index :loadmore="loadmore" :thead="thead" :data.sync="data">
        ...
        <template slot-scope="scope">
          <span @click="$router.push('/edit/check/'+scope.row._id)" class="link" v-if="scope.prop==='no'">
            {{scope.row.no}}
          </span>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      try {
        this.data = await this.$ajax(
          "/order/pending/finish?limit=" + this.limit
        );
      } catch (error) {}
      this.loadingText = "";
    },
    ...
    async loadmore() {
      return await this.$ajax.post("/order/pending/finish", {
        limit: this.limit,
