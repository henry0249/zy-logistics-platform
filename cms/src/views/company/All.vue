  ...
  <loading-box v-model="loadingText" class="g-order-create">
    <div class="tab-box">
      ...
      <div>
        ...
        <all-cart :data.sync="cartData" :init-data="initCartData" :isInvoice="isInvoice"></all-cart>
...
<script>
  ...
  export default {
      ...
      sub() {
        if (this.checkMethods()) {
          ...
          if (this.edit) {} else {
            ...
            if (this.$route.query.payUserType === 'company') {
              obj.company = this.cartData.company._id;
            } else if (this.$route.query.payUserType === 'user') {
    ...
    created() {
      ...
      this.$set(this.initCartData.from, 'userType', this.$route.query.payUserType);
      if (this.isInvoice) {
        this.invoice();
      } else {
        this.receive();
