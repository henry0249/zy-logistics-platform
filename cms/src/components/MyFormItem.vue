      ...
      <slot>
        ...
        <el-dropdown v-else-if="$attrs.dropdown!==undefined" v-bind="$attrs" :size="size||$parent.size" @command="change">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="item in json2arr($attrs.options)" :key="item._id||item.value || item.id">{{item.name || item.label}}</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
  ...
  export default {
    ...
    methods: {
      change(val) {
        ...
        if (this.$attrs.dropdown!==undefined) {
          this.$emit("input", val);
        }
      },
      areaChange(val) {
